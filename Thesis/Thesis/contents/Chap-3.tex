...
+                 & Rotate the current direction by \SI{45}{\degree} clockwise           \\
...
4 &
  - &
  \begin{tabular}[c]{@{}l@{}}Rotate the current direction by \SI{45}{\degree}\\ counterclockwise\end{tabular} \\
5 &
  ++ &
  Rotate the current direction by \SI{90}{\degree}  clockwise \\
6 &
  -- &
  \begin{tabular}[c]{@{}l@{}}Rotate the current direction by \SI{90}{\degree}\\ counterclockwise\end{tabular} \\
7 &
  fF &
  2, then 1 \\
8 &
...
\begin{table}[]
\centering
\caption{Template class object initialisation parameters and prerequisites}
\label{tab:temppar}
\begin{tabular}{@{}llc@{}}
\toprule
...
\textit{e\_s}                          & \begin{tabular}[c]{@{}l@{}}The side length of the element in\\ \textbackslash{}si\{mm\}\end{tabular}  & $>0$                  \\
...
\textit{d\_mag}                        & The applied displacement in \textbackslash{}si\{mm\}                                                  &                       \\
\textit{p\_mag}                        & \begin{tabular}[c]{@{}l@{}}The applied internal pressure in\\ \textbackslash{}si\{MPa\}\end{tabular}  &                       \\ \bottomrule
\end{tabular}
\end{table}
...
Analysis approach parameters that need to be specified are outlined in Table~\ref{tab:temppar}. Prerequisites for the parameters are included where applicable.
...
\end{tabular}
\caption[Template parameters]{Calculated and internal template class object parameters}
