...
\begin{figure}[H]
	...
	\caption{Soft robot constructed from a tetrahedral mesh \cite{Rieffel2013}}
	\label{fig:mesh}
\end{figure}

\subsection{Gaussian Mixtures}

The Gaussian mixtures representation \cite{Pernkopf2005} uses a density field analogy to a level-set method. The density field is initialised as having zero density. Points of density with Gaussian falloff are added within the field. The points may have positive or negative weights. Positive weights contribute to the density, while negative weights subtract from it. If a single point was used, a solid sphere would be the thresholded result. \cite{Hiller2012}
...
\begin{figure}[H]
	...
	\caption{Soft body represented by a Gaussian density mixture \cite{Hiller2012}}
	\label{fig:gauss}
\end{figure}
